...
class IAMPolicyFactory:
    ...
    def create_ec2_policy(
        name: str, actions: List[str] = None, resources: List[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        return {
            "resource": [
                {"null_resource": [{f"iam_policy_{name}": [{"triggers": triggers}]}]}
            ]
        }
    ...
    def create_kubernetes_policy(name: str, cluster_name: str = None) -> Dict[str, Any]:
        ...
        resources = (
            [f"arn:aws:eks:*:*:cluster/{cluster_name}"] if cluster_name else ["*"]
        )
...
class IAMRoleFactory:
    ...
    def create_service_role(
        name: str, service: str, policies: List[str] = None, tags: Dict[str, str] = None
    ) -> Dict[str, Any]:
        """
        ...
        return {
            "resource": [
                {"null_resource": [{f"iam_role_{name}": [{"triggers": triggers}]}]}
            ]
        }

    @staticmethod
    def create_user_role(
        name: str, policies: List[str] = None, tags: Dict[str, str] = None
    ) -> Dict[str, Any]:
        """
...
class IAMUserFactory:
    ...
    def create_service_user(
        ...
        return {
            "resource": [
                {"null_resource": [{f"iam_user_{name}": [{"triggers": triggers}]}]}
            ]
        }
...
class IAMModule:
    ...
    def add_kubernetes_rbac(
        self, cluster_name: str, tags: Dict[str, str] = None
    ) -> "IAMModule":
        """
        ...
        node_role = IAMRoleFactory.create_service_role(
            f"{cluster_name}_node_role", "ec2", [f"{cluster_name}_admin_policy"], tags
        )
        self.resources.append(node_role)

        # Usuario de servicio para CI/CD
        service_user = IAMUserFactory.create_service_user(
            f"{cluster_name}_cicd_user", [f"{cluster_name}_admin_policy"], tags
        )
        self.resources.append(service_user)

        return self

    def add_network_rbac(
        self, vpc_name: str, tags: Dict[str, str] = None
    ) -> "IAMModule":
        """
        ...
        network_admin_role = IAMRoleFactory.create_user_role(
            f"{vpc_name}_network_admin", [f"{vpc_name}_network_policy"], tags
        )
        self.resources.append(network_admin_role)

        return self

    def add_compute_rbac(
        self, compute_name: str, tags: Dict[str, str] = None
    ) -> "IAMModule":
        """
