...
class TerraformPlanValidator:
    ...
    def validate_vpc_resources(self) -> bool:
        ...
        vpc_resources = [
            ...
            if r.get('type') == 'null_resource' and 
            'vpc' in r.get('values', {}).get('triggers', {}).get('resource_type', '')
        ]
    ...
    def validate_subnet_resources(self) -> bool:
        ...
        subnet_resources = [
            r for r in resources 
            if r.get('type') == 'null_resource' and 
            'subnet' in r.get('values', {}).get('triggers', {}).get('resource_type', '')
        ]
        
        if len(subnet_resources) < 2:
            print(f"se esperaban al menos 2 subredes, encontradas: {len(subnet_resources)}")
            return False
    ...
    def validate_kubernetes_resources(self) -> bool:
        ...
        k8s_cluster = [
            r for r in resources 
            if r.get('type') == 'null_resource' and 
            'kubernetes_cluster' in r.get('values', {}).get('triggers', {}).get('resource_type', '')
        ]
        ...
        k8s_nodes = [
            r for r in resources 
            if r.get('type') == 'null_resource' and 
            r.get('values', {}).get('triggers', {}).get('resource_type', '') in 
            ['kubernetes_master', 'kubernetes_node']
        ]
    ...
    def validate_iam_resources(self) -> bool:
        ...
        for resource in iam_resources:
            iam_type = resource.get('values', {}).get('triggers', {}).get('resource_type', '')
