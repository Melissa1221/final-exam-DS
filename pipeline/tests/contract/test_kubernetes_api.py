...
class TestKubernetesApiContract:
    ...
    def test_get_cluster_info(self, pact):
        ...
        (pact
         .given('cluster exists and is ready')
    ...
    def test_create_namespace(self, pact):
        ...
        with pact:
            response = requests.post(
                'http://localhost:8443/api/v1/namespaces',
                json=namespace_request
            )
    ...
    def test_get_pods_status(self, pact):
        ...
        with pact:
            response = requests.get('http://localhost:8443/api/v1/pods?namespace=default')
            assert response.status_code == 200
