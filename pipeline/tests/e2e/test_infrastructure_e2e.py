...
class TestInfrastructureE2E:
    ...
    def test_prometheus_metrics(self):
        ...
        metrics_response = self.session.get(
            f"{prometheus_url}/api/v1/query?query=up", timeout=10
        )
    ...
    def test_full_infrastructure_integration(self):
        ...
        failed_services = [
            name for name, status in service_status.items() if not status
        ]
    ...
    def test_monitoring_data_flow(self):
        ...
        metrics_response = self.session.get(
            f"{prometheus_url}/api/v1/query?query=prometheus_build_info", timeout=10
        )
        assert metrics_response.status_code == 200
